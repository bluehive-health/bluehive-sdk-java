...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString().orElseThrow { BlueHiveInvalidDataException("Value is not a string") }
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw BlueHiveInvalidDataException("`$name` is not set")
            is JsonNull -> throw BlueHiveInvalidDataException("`$name` is null")
            else -> throw BlueHiveInvalidDataException("`$name` is invalid, received $this")
        }
